...
<script>
    //이전 버튼 이벤트
    function fn_prev(page, range, rangeSize, cate) {
        var page = ((range - 2) * rangeSize) + 1;
        var range = range - 1;
        var url = "${pageContext.request.contextPath}/place/categoryPlace";
        url = url + "?page=" + page;
        url = url + "&range=" + range;
        url = url + "&cate=" + cate;
        location.href = url;
    ...
    //페이지 번호 클릭
    function fn_pagination(page, range, rangeSize, cate) {
        var url = "${pageContext.request.contextPath}/place/categoryPlace";
        url = url + "?page=" + page
        url = url + "&range=" + range;
        url = url + "&cate=" + cate;
    ...
    //다음 버튼 이벤트
    function fn_next(page, range, rangeSize, cate) {
								...
								<c:when test="${not empty sessionScope.user}">
									<c:choose>
										<c:when test="${bookList.contains(placeList[i].PLACE_NO)}">
											<a
												href="${pageContext.request.contextPath}/place/catebookmarkRemove?&place_no=${placeList[i].PLACE_NO}&cate=${cate}"
										...
										<c:otherwise>
											<a
												href="${pageContext.request.contextPath}/place/catebookmarkAdd?&place_no=${placeList[i].PLACE_NO}&cate=${cate}"
												class="bookmarkBtn" data-place-id="${placeList[i].PLACE_NO}"
												style="color: '#ffffff';">
			...
			<div id="paginationBox">
			    <ul class="pagination">
			        ...
			        <c:forEach begin="${pagination.startPage}" end="${pagination.endPage}" var="idx">
			            <li class="page-item <c:out value="${pagination.page == idx ? 'active' : ''}"/> "><a class="page-link" href="#" onClick="fn_pagination('${idx}', '${pagination.range}', '${pagination.rangeSize}', '${pagination.cate }')"> ${idx} </a></li>
			        </c:forEach>
