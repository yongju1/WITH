...
<link rel="stylesheet" href="/resources/css/adminUserInfo.css">
<title>WITH</title>
</head>
...
<script>
    //이전 버튼 이벤트
    function fn_prev(page, range, rangeSize, keyword, order) {
        var page = ((range - 2) * rangeSize) + 1;
        var range = range - 1;
        var url = "${pageContext.request.contextPath}/admin/adminUserInfo";
        url = url + "?page=" + page;
        url = url + "&range=" + range;
        url = url + "&order=" + order;
    ...
    //다음 버튼 이벤트
    function fn_next(page, range, rangeSize, keyword, order) {
						...
						<div class="list-title">
							...
							<div class="list-title2">
								<div class="search_bar">
									<form action="/admin/adminUserInfo" method="get">
										<div style="display: flex;">
											<input class="form-control" type="text" name="keyword"
												placeholder="ID, 이름 검색" value="${pagination.keyword}">
					...
					<tbody>
						<c:forEach items="${userList}" var="m">
							<tr>
								...
								<td>${m.user_name}</td>
								<td>${m.user_email}</td>
								<td><fmt:formatDate value="${m.user_date}"
										pattern="yyyy/MM/dd" /></td>
								<td><form action="/admin/deleteUser" method="post"
										onsubmit="return confirmDelete()">
										<input type="hidden" name="user_no" value="${m.user_no}" />
										<button type="submit"
...
<script>
	...
	/* document.getElementById("lang").onchange = function() {
		var order = this.value === "old" ? "DESC" : "ASC";
		location.href = "/admin/userListOrdered?order=" + order;
	}; */
	...
	document.getElementById("lang").onchange = function() {
		...
		location.href = "/admin/adminUserInfo?order=" + order;
	};
</script>
<footer> </footer>
